...
parser.add_argument("-trim", "--trim", type=bool, default=True)
...
if __name__ == '__main__':
    ...
    if args.trim:
        ...
        captionFolder = 'captions/'
        video_width = 456
        video_number_limit = 200

        if not (os.path.exists(outputFolder)):
            # Create directory
            subprocess.call('mkdir -p ' + outputFolder, shell=True)
        print(os.listdir(inputFolder))
        print('Total Length of captions is', len(os.listdir(inputFolder)))
        for video_filename in sorted(os.listdir(inputFolder))[:video_number_limit]:
            video_path = os.path.join(inputFolder, video_filename)
            caption_filename = video_filename.replace('.mp4', '.en.vtt')
            caption_path = os.path.join(captionFolder, caption_filename)

            num = int(video_filename.replace('.mp4', ''))
            captions = WebVTT().read(caption_path)
            for idx, caption in enumerate(captions):
                output_path = os.path.join(outputFolder, "{}-{}.mp4".format(num, str(idx).rjust(3, '0')))
                t = int(round(get_sec(caption.start) - get_sec(caption.end)))
                print('index: ', idx, 'Text: ', caption.text, 'Time: ', t)
                cmd = 'ffmpeg -i {vid_path} -vf scale={wid}:256 -ss {start} -t {duration} -acodec copy {out_path}'. \
                    format(vid_path=video_path, wid=video_width, start=caption.start, duration=t, out_path=output_path)
    ...
    if (args.extract_audio):
        ...
        for file in filelist:
            cmd = 'ffmpeg -i ' + file + ' -ab 160k -ac 1 -ar 16000 -vn ' + outputFolder + file[len(inputFolder): -len(
                '.mp4')] + '.wav'
            subprocess.call(cmd, shell=True)
            ...
            for idx, frame in enumerate(big_list[key]):
                ...
                if (idx > 0):
                    ...
                    for j in range(start, end):
                        new_kp[j, :, :] = new_kp[start - 1, :, :] + ((new_kp[end, :, :] - new_kp[start - 1, :, :]) * (
